...
const SearchTab = ({ closeSearch }) => {
    ...
    useEffect(() => {
        const fetchDefaultItems = async () => {
            try {
                const response = await fetch('https://clothing-brand-backend.vercel.app/posts');
    ...
    const handleSearchChange = async (event) => {
        ...
        if (searchQuery.trim()) {
            try {
                const response = await fetch(`https://clothing-brand-backend.vercel.app/posts?q=${searchQuery}`);
                const data = await response.json();
